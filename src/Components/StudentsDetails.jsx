...
const StudentsDetails = () => {
  ...
  useEffect(() => {
    if (id) {
      // for updating the existing student, first checking whether the id exists,then it means we want to update existing studentDetails,so we want to fetch studentDetails through id,when id matches,we will get that studentDetails
  ...
  const submitHandler = () => {
    if (id) {
      ...
      for (let i = 0; i < studentDetails.length; i++) {
        ...
        if (id.toString() === studentDetails[i].id.toString()) {
          //if id matches,storing taht index in i
    ...
    <div>
      <div className="detail__container">
        <div className="form__student__details mr-20">
          <input type="text" id="name" name="name" className="form-control" value={currentStudentDetails.Name} onChange={(e) => changeStudentDetails(e, "Name")} />
          <label className="label" htmlFor="name">
            Name
          </label>
        </div>
        <div className="form__student__details">
          <input type="text" id="age" name="age" className="form-control input__left" value={currentStudentDetails.Age} onChange={(e) => changeStudentDetails(e, "Age")} />
          <label className="label label__left" htmlFor="age">
            Age
          </label>
        </div>
      </div>
      <div className="detail__container">
        <div className="form__student__details mr-20">
          <input type="text" id="course" name="course" className="form-control" value={currentStudentDetails.Course} onChange={(e) => changeStudentDetails(e, "Course")} />
          <label className="label" htmlFor="course">
            Course
          </label>
        </div>
        <div className="form__student__details">
          <input type="text" id="batch" name="batch" className="form-control input__left" value={currentStudentDetails.Batch} onChange={(e) => changeStudentDetails(e, "Batch")} />
          <label className="label label__left" htmlFor="batch">
            Batch
          </label>
        </div>
      </div>
      <div className="form_btn_container">
        <button className="form_btn cancel_btn_color" onClick={() => navigate("/students")}>
          Cancel
        </button>
        <button className="form_btn update__btn__color" onClick={submitHandler}>
          {id ? "Update" : "Submit"}
        </button>
      </div>
    </div>
